...
import { navHrefs, productSpecRows } from '../data/catalog'
...
function TrustItem({ trustKey }: { trustKey: (typeof productTrustKeys)[number] }) {
  const text = useHt(`product.trust.${trustKey}`)
  return (
    <Stack direction="row" spacing={0.75} sx={{ alignItems: 'center' }}>
      <CheckCircleRounded sx={{ fontSize: 18, color: brand.plantGreen }} />
      <Typography sx={{ fontSize: '0.82rem', fontWeight: 600, color: brand.graphite }}>{text}</Typography>
    </Stack>
  )
}
          ...
          <Reveal>
            ...
            <Stack direction="row" useFlexGap sx={{ flexWrap: 'wrap', gap: { xs: 1.5, sm: 2.5 } }}>
              {productTrustKeys.map((key) => (
                <TrustItem key={key} trustKey={key} />
              ))}
            </Stack>
          </Reveal>
        </Grid>
